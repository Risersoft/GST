...
Public Class frmInvoiceItemPurch
    ...
    Public Overloads Function BindModel(NewModel As clsFormDataModel) As Boolean
        ...
        Me.cmb_ITCReversal_4243.ValueList = fMat.Model.ValueLists("ITCReversal").ComboList
    ...
    Private Sub txt_TXVal_Leave(sender As Object, e As EventArgs) Handles txt_TXVal.Leave
        CalculateTaxableVal()
    End Sub

    Private Sub CalculateTaxableVal()
        'txt_TXVal.Value = Math.Round(myUtils.cValTN(txt_Qty.Value) * myUtils.cValTN(txt_BasicRate.Value), 2)
        myRow("basicrate") = If(myUtils.cValTN(txt_Qty.Value) > 0, myUtils.cValTN(txt_TXVal.Value) / myUtils.cValTN(txt_Qty.Value), 0)
        CalculateTaxVal()
    End Sub
