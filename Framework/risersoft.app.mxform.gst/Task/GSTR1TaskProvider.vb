...
Public Class GSTR1TaskProvider
    ...
    Public Overrides Async Function ExecuteServer(rTask As DataRow, input As clsProcOutput) As Task(Of clsProcOutput)
        ...
        Select Case myUtils.cStrTN(rTask("actionsubtype")).Trim.ToLower
            ...
            Case "deleteimportonline"
                Dim oProc As New clsGSTInvoiceSale(myContext)
                oRet = oProc.DeletePeriod($"GstRegID={GstRegID}", $"campusid in (select campusid from campus where GstRegID={GstRegID})", $"ReturnPeriodID={ReturnPeriodID}", "isnull(gstnflag,0)=0", False)
                oRetMail = Await mailer.SendGenericMail(myUtils.cStrTN(rTask("username")), "Delete", oRet.Message)
            ...
            Case "pdf"
                ...
                Dim fRow As DataRow = myContext.AppModel.FrmPrnRowKey("crpreturngstr1", "")
            ...
            Case "uploadm"
                ...
                For Each str1 As String In arr
                    ...
                    End Try
                    If oRet2.Success Then oRet2 = oProc.CreateStatusTask(rTask, myUtils.cValTN(str1), ReturnPeriodID)
            ...
            Case "reconcile"
                ...
                If oRet.Success Then
                    Dim oProc2 As New clsGSTNReturnGSTR1(myContext)
