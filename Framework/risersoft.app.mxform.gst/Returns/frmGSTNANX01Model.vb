...
<DataContract>
Public Class frmGSTNANX01Model
    ...
    Protected Friend Overloads Function GenerateData(ByVal CompanyID As Integer, ReturnPeriodID As Integer) As DataSet
        ...
        dic.Add("gstreg", "Select GSTRegID, CompName, GSTIN, Descrip, convert(money,0) As val, convert(money,0) As iamt, convert(money,0) As camt, convert(money,0) As samt, convert(money,0) As csamt, convert(money,0) As txval from GstListGSTReg() where  CompanyID = " & CompanyID)
        ...
        sum(isnull(factor*Factor2*csamt,0)) As csamt,sum(isnull(val,0)) as val from GstListInvoiceANX() 
        where isnull(anx01section,'')<>'' and isnull(StatusNum,0)=2  and CancelDate is NULL and CompanyID={0} and ReturnPeriodID={1}
                ...
                Select Case dataKey.Trim.ToLower
                    ...
                    Case "filesign"
                        ...
                        If Not nr Is Nothing Then
                            ...
                            End If
                        End If
                    Case "delete", "deletecp", "deleteimportonline"
                        Dim dicParams As New Dictionary(Of String, String)
                        oRet = TaskProviderFactory.CheckAddTask(myContext, "anx01", dataKey, CompanyID, Params, queueName, dicParams)
                        If oRet.Success Then oRet.Description = oRet.GetCalcRows("task")(0)("apitaskid").ToString
                    Case "reconcile"
                        Dim filename As String = myContext.Controller.CalcRLSId.ToString & "-ANX01-Reconcile-" & Replace(Now.ToLongTimeString, ":", "").Replace(" ", "") & ".xlsx"
                        Dim dicParams As New Dictionary(Of String, String)
                        dicParams("filename") = filename
                        oRet = TaskProviderFactory.CheckAddTask(myContext, "anx01", dataKey, CompanyID, Params, queueName, dicParams)
                        If oRet.Success Then oRet.Description = oRet.GetCalcRows("task")(0)("apitaskid").ToString
                    Case "payload"
                        Dim filename As String = myContext.Controller.CalcRLSId.ToString & "-ANX01-" & Replace(Now.ToLongTimeString, ":", "").Replace(" ", "") & ".zip"
                        Dim dicParams As New Dictionary(Of String, String)
                        dicParams("filename") = filename
                        oRet = TaskProviderFactory.CheckAddTask(myContext, "anx01", dataKey, CompanyID, Params, queueName, dicParams)
                        If oRet.Success Then oRet.Description = oRet.GetCalcRows("task")(0)("apitaskid").ToString
                    Case "excel"
                        Dim filename As String = myContext.Controller.CalcAccountName & "-ANX01-" & Replace(Now.ToLongTimeString, ":", "").Replace(" ", "") & ".xlsx"
                        Dim dicParams As New Dictionary(Of String, String)
                        dicParams("filename") = filename
                        oRet = TaskProviderFactory.CheckAddTask(myContext, "anx01", dataKey, CompanyID, Params, queueName, dicParams)
                        If oRet.Success Then oRet.Description = oRet.GetCalcRows("task")(0)("apitaskid").ToString
                    Case "pdf"
                        Dim filename As String = myContext.Controller.CalcAccountName.ToString & "-ANX01-" & Replace(Now.ToLongTimeString, ":", "").Replace(" ", "") & ".pdf"
                        Dim dicParams As New Dictionary(Of String, String)
                        dicParams("filename") = filename
                        oRet = TaskProviderFactory.CheckAddTask(myContext, "anx01", dataKey, CompanyID, Params, queueName, dicParams)
                        If oRet.Success Then oRet.Description = oRet.GetCalcRows("task")(0)("apitaskid").ToString
                    Case "template", "gstn_error"
                        Dim filename As String = myContext.Controller.CalcRLSId.ToString & "-ANX01-" & Replace(Now.ToLongTimeString, ":", "").Replace(" ", "") & ".xlsx"
                        Dim dicParams As New Dictionary(Of String, String)
                        dicParams("filename") = filename
                        oRet = TaskProviderFactory.CheckAddTask(myContext, "anx01", dataKey, CompanyID, Params, queueName, dicParams)
                        If oRet.Success Then oRet.Description = oRet.GetCalcRows("task")(0)("apitaskid").ToString
